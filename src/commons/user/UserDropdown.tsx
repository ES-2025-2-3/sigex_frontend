...
const UserDropdown: React.FC = () => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div 
          className={`absolute right-0 mt-3 w-64 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden z-[5000] transition-all duration-200 ease-out ${
        ...
        >
          <div className="p-4 bg-brand-dark text-white flex items-center gap-3">
             <div className="w-10 h-10 rounded-full border border-white/20 overflow-hidden flex-shrink-0 bg-brand-blue flex items-center justify-center">
                {user.photoUrl ? (
                   <img src={user.photoUrl} alt="" className="w-full h-full object-cover" />
                ) : (
                   <span className="text-xs font-bold">JA</span>
                )}
             </div>
             <div className="overflow-hidden">
                <p className="text-sm font-bold truncate">{user.name}</p>
                <p className="text-[10px] text-gray-400 font-medium truncate opacity-80">{user.email}</p>
             </div>
          </div>
