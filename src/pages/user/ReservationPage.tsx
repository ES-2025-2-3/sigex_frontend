...
function ReservationPage() {
    ...
    const filteredReservations = reservations.filter((r) => r.event.toLowerCase().includes(search.toLowerCase())).sort((a,b) => {
        if (a.status === "CANCELED" && b.status === "ACTIVE") return 1;
        if(a.status === "ACTIVE" && b.status === "CANCELED") return -1;
        return 0;
    })
    
    //funcao que retira os eventos com id recebido para o proximo estado
    const handleCancel = (id: string) => {setReservations((prev) => prev.map((r) => r.id === id? {...r, status:"CANCELED"} : r));};
                            ...
                            <table className="min-w-full text-sm text-left text-gray-600">
                                <thead className="bg-gray-100 text-gray-700 uppercase text-xs">
                                    <tr className = "relative hover:bg-gray-50 transition">
                                        ...
                                        <th className="px-6 py-4 w-34">Status</th>
                                        <th className="px-6 py-4 text-center">Ações</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredReservations.map((r) => (
                                    <tr key={r.id} className={`transition ${r.status === "CANCELED" ? "opacity-50 bg-gray-50" : "hover:bg-gray-50"}`}>
                                        ...
                                        <td className="px-6 py-4 w-34">{r.status === "CANCELED"?(
                                        ...
                                        <button
                                            ...
                                            className={`transition ${r.status === "CANCELED"? "text-gray-300 cursor-not-allowed" : "text-red-500 hover:text-red-600"}`}>
                                            <FaTrash size={18} className="pointer-events-none"/>
                                            </button>
                                        </td>
                                    </tr>
                                    ))}
