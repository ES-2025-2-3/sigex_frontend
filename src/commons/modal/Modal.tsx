...
export default function Modal({ isOpen, title, onClose, children }: ModalProps) {
  ...
  return (
    <div 
      className={`fixed inset-0 z-[2000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity duration-300 ease-in-out ${
    ...
    >
      <div
        className={`bg-white w-full max-w-[600px] max-h-[85vh] overflow-y-auto rounded-2xl shadow-2xl p-6 relative flex flex-col transition-all duration-300 ease-out ${
          isRendered ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 translate-y-8"
        }`}
